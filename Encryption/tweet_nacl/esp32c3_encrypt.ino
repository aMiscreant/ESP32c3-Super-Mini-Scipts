...
void runEncryptionDecryption() {
  ...
  size_t paddedLen = msgLen + 32; // Enough for padding, 32 extra bytes

  uint8_t* msgPadded = (uint8_t*)calloc(1, paddedLen);  
  uint8_t* cipher = (uint8_t*)calloc(1, paddedLen);     
  uint8_t* decrypted = (uint8_t*)calloc(1, paddedLen);  
  ...
  memset(msgPadded, 0, paddedLen);  // Ensure no leftover data in the allocated space
  memcpy(msgPadded + 32, msg, msgLen); // Copy original message into padded space
  delay(5000);
  ...
  //printHex("Ciphertext:", cipher + 16, msgLen + 16); // Exclude padding from ciphertext
  printHex("Ciphertext:", cipher + 16, msgLen);
  delay(5000);
...
cleanup:
  Serial.println("Zeroing memory before freeing...");
  memset(msgPadded, 0, paddedLen); // Explicitly zero the buffer
  memset(cipher, 0, paddedLen);
...
void setup() {
  Serial.begin(115200);
  delay(1000); // Allow serial to stabilize on ESP32
  Serial.println("Starting TweetNaCl test loop...");
}

void loop() {
  runEncryptionDecryption();
  delay(50000); // every 5 seconds
}
